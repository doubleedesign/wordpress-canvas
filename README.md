...
> [!NOTE]
> If setting up this repo to work on WordPress Canvas itself, please follow the manual setup instructions for the time being. For use as a template repo, automated setup for new sites has moved to my [PowerPress](https://github.com/doubleedesign/powerpress) project.

> [!IMPORTANT]  
> If you are using this repo as a project template, follow the [instructions](#using-this-repo-as-a-project-template) below about that before setting up.
...
2. if using local copies of the above-listed plugins, ensure they are up-to-date with local Composer configuration and dependencies in each project directory. (Remove `--no-dev` if you also intend to do things like unit testing in those projects.)
    ...
    ```powershell
     $env:COMPOSER = "composer.dev.json"; composer update --prefer-source --no-dev
    composer dump-autoload -o
    ```
	
    If a project does not have a `composer.dev.json` file, close the terminal window and open a new one (to clear the temporary environment variable) before running the basic commands:
...
> [!TIP]
> See my [PowerPress](https://github.com/doubleedesign/powerpress) project for a way to automate this if you're on Windows.
